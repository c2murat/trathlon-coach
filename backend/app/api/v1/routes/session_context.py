...
def get_session_context(current_user:AuthenticatedUser=Depends(get_current_user),session:Session=Depends(get_db_session),requested_athlete_id:str|None=Header(default=None,alias="X-TriCoach-Athlete-Id"))->SessionContextResponse:
 ...
 memberships=list(session.scalars(select(UserAthleteMembership).join(UserAthleteMembership.athlete_profile).where(UserAthleteMembership.user_id==current_user.id,UserAthleteMembership.is_active.is_(True),AthleteProfile.deleted_at.is_(None))).all())
 views=[AthleteMembershipResponse(athlete_id=m.athlete_profile_id,label=m.athlete_profile.display_name,role=m.role,is_default=m.is_default,capabilities=[c.value for c in capabilities_for_role(m.role)]) for m in memberships]
