...
def create_goal(payload:CompetitionGoalCreateRequest,current:CurrentAthleteContext=Depends(manage),session:Session=Depends(get_db_session)):return response(execute(session,lambda:CompetitionGoalApplication(session).create(current.athlete_id,current.user_id,current.athlete_profile.timezone,payload.model_dump())))
...
def update_goal(goal_id:UUID,payload:CompetitionGoalUpdateRequest,current:CurrentAthleteContext=Depends(manage),session:Session=Depends(get_db_session)):return response(execute(session,lambda:CompetitionGoalApplication(session).update(current.athlete_id,goal_id,current.athlete_profile.timezone,payload.model_dump(exclude_unset=True))))
