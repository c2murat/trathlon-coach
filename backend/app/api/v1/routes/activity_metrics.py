...
def own(session,athlete_id,activity_id):return session.scalar(select(CompletedActivity).options(selectinload(CompletedActivity.streams),selectinload(CompletedActivity.laps)).where(CompletedActivity.id==activity_id,CompletedActivity.athlete_id==athlete_id,CompletedActivity.deleted_at.is_(None)))
...
def get_metrics(activity_id:UUID,current_athlete:CurrentAthleteContext=Depends(get_current_athlete),session:Session=Depends(get_db_session)):
