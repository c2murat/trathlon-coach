...
read_strava=require_athlete_capability(AthleteCapability.READ_STRAVA_INTEGRATION);run_evidence=require_athlete_capability(AthleteCapability.RUN_STRAVA_EVIDENCE);delete_location=require_athlete_capability(AthleteCapability.DELETE_STRAVA_LOCATION_EVIDENCE)
...
async def start_evidence(body:EvidenceRequest,current_user:AuthenticatedUser=Depends(get_current_user),current_athlete:CurrentAthleteContext=Depends(run_evidence),manager:StravaActivityEvidenceManager=Depends(get_strava_evidence_manager)):
 try:job=await run_in_threadpool(manager.create_job,current_user.id,athlete_id=current_athlete.athlete_id,activity_ids=body.activity_ids,include_laps=body.include_laps,include_streams=body.include_streams,include_location=body.include_location)
...
def evidence_status(job_id:UUID,current_user:AuthenticatedUser=Depends(get_current_user),current_athlete:CurrentAthleteContext=Depends(read_strava),manager:StravaActivityEvidenceManager=Depends(get_strava_evidence_manager)):
 try:job=manager.job_for_user(current_user.id,job_id,athlete_id=current_athlete.athlete_id)
...
def cleanup_location(current_user:AuthenticatedUser=Depends(get_current_user),current_athlete:CurrentAthleteContext=Depends(delete_location),manager:StravaActivityEvidenceManager=Depends(get_strava_evidence_manager)):
 return JSONResponse({"deleted_count":manager.cleanup_location_for_user(current_user.id,athlete_id=current_athlete.athlete_id)},headers=HEADERS)
@router.get("/activities/{activity_id}/evidence")
def activity_evidence(activity_id:UUID,current_athlete:CurrentAthleteContext=Depends(read_strava),session:Session=Depends(get_db_session),settings:Settings=Depends(get_settings)):
 value=ActivityEvidenceQuery(session,location_enabled=settings.activity_location_stream_retention_enabled,max_samples=settings.activity_stream_max_samples).for_athlete(current_athlete.athlete_id,activity_id)
