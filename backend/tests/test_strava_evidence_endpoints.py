...
from app.db.models import ActivityEvidenceState,ActivityStream,AthleteProfile,CompletedActivity,User,UserAthleteMembership
from app.db.session import get_db_session
from app.integrations.strava.activity_evidence import EvidenceJobView,EvidenceJobNotFoundError
from app.main import create_app
from app.api.v1.routes.strava_evidence import read_strava,run_evidence
class Manager:
 ...
 def job_for_user(self,user,j,*,athlete_id=None):
...
def test_evidence_job_endpoints_are_safe_and_bounded():
 app=create_app();m=Manager();app.dependency_overrides[get_strava_evidence_manager]=lambda:m;context=SimpleNamespace(athlete_id=uuid4());app.dependency_overrides[read_strava]=lambda:context;app.dependency_overrides[run_evidence]=lambda:context
...
def test_evidence_read_is_owned_bounded_and_coordinate_free_when_disabled():
 ...
 with Session(engine) as s:
  user=User(id=LOCAL_MVP_USER_ID,email="a@x.invalid",normalized_email="a@x.invalid",auth_subject="a");athlete=AthleteProfile(display_name="Test athlete");activity=CompletedActivity(athlete=athlete,external_activity_id="1",source_summary="strava",sport="running",name="Run",start_at=now,timezone="UTC",elapsed_time_s=1);s.add_all([user,athlete,activity]);s.flush();s.add(UserAthleteMembership(user_id=user.id,athlete_profile_id=athlete.id,role="owner",is_active=True,is_default=True));s.add(ActivityStream(completed_activity_id=activity.id,stream_type="time",original_sample_count=2000,sample_count=2,values=[0,1],fetched_at=now,retention_class="standard",checksum="x",version=1));s.add(ActivityStream(completed_activity_id=activity.id,stream_type="latlng",original_sample_count=2,sample_count=2,values=[[40,-3],[41,-4]],fetched_at=now,retention_class="location",checksum="y",version=1));s.add(ActivityEvidenceState(completed_activity_id=activity.id,status="succeeded",streams_fetched_at=now,location_retained=True));s.commit();aid=activity.id
 app=create_app()
 def db():
