...
def goal(**values):
 base={"name":"Objetivo","event_date":date.today()+timedelta(days=30),"timezone":"Europe/Madrid","event_category":"running","event_format":"10k","priority":"A","segments":[{"position":1,"sport":"run","distance_m":10000}]}
 return CompetitionGoalInput.model_validate({**base,**values})
@pytest.mark.parametrize("priority",["A","B","C"])
def test_goal_priorities_and_single_sport(priority):assert goal(priority=priority).segments[0].distance_m==10000
@pytest.mark.parametrize("category",["running","cycling","swimming","triathlon","duathlon","aquathlon"])
def test_all_multisport_categories(category):assert goal(event_category=category).event_category==category
@pytest.mark.parametrize("values",[{"priority":"D"},{"target_finish_time_seconds":0},{"timezone":"Invalid/Zone"},{"segments":[{"position":1,"sport":"run","distance_m":-1}]},{"segments":[{"position":2,"sport":"run","distance_m":1000}]}])
def test_goal_rejects_invalid_values(values):
 with pytest.raises(ValidationError):goal(**values)
def test_repeated_ordered_segments_are_allowed_without_rigid_sequence():
 item=goal(event_category="duathlon",event_format="special",segments=[{"position":1,"sport":"run","distance_m":5000},{"position":2,"sport":"run","distance_m":2000},{"position":3,"sport":"bike","distance_m":20000}]);assert [x.sport for x in item.segments]==["run","run","bike"]
