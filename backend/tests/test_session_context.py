...
def add(e,role="owner",default=False,active=True):
 with Session(e) as s:
  token=str(uuid4());u=User(email=f"{token}@test",normalized_email=f"{token}@test",auth_subject=token);a=AthleteProfile(user=u);s.add_all([u,a,UserAthleteMembership(user_id=LOCAL_MVP_USER_ID,athlete_profile=a,role=role,is_default=default,is_active=active)]);s.commit();return a.id
...
def test_multiple_defaults_and_unknown_role(context_client):
 c,e=context_client;add(e,default=True);add(e,default=True);assert c.get("/session/context").json()["detail"]["code"]=="athlete_default_configuration_invalid";assert capabilities_for_role("unknown")==()
