...
interface AuthContextValue{status:AuthStatus;user:AuthenticatedUser|null;login(credentials:LoginCredentials):Promise<void>;register(input:RegistrationInput):Promise<void>;logout():Promise<void>;refreshUser():Promise<void>}
...
export function AuthProvider({children,client=apiClient}:{children:ReactNode;client?:ApiClient}){
 ...
 const register=useCallback(async(input:RegistrationInput)=>{if(!client.register)throw new Error("registration_unavailable");const value=await client.register(input);setUser(value);setStatus("authenticated");navigate(value.account_plan==="athlete"?"/settings/athlete-profile":"/dashboard")},[client]);
 ...
 const value=useMemo(()=>({status,user,login,register,logout,refreshUser}),[status,user,login,register,logout,refreshUser]);
