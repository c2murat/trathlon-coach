...
export function AthleteContextProvider({children,client=apiClient}:{children:ReactNode;client?:ApiClient}){
 ...
 const refreshContext=useCallback(async()=>{const generation=++requestGeneration.current;setLoading(true);setError(null);try{
  if(!client.sessionContext)throw new Error("session_context_unavailable");
  const context=await client.sessionContext();if(!mounted.current||generation!==requestGeneration.current)return;
 ...
 }catch(cause){if(!mounted.current||generation!==requestGeneration.current||isAbortError(cause))return;setError("No se ha podido cargar el contexto de usuario.");transport.setActiveAthlete(null)}finally{if(mounted.current&&generation===requestGeneration.current)setLoading(false)}},[client,transport]);
