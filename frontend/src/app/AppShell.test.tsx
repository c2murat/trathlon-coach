...
it("marks profile and reference actions as denied for read-only capabilities",async()=>{const viewer={...session,athletes:[{...session.athletes[0],role:"viewer",capabilities:["read_athlete_data","read_strava_integration"]}]};renderShell("/settings/performance-profile",viewer);const heading=await screen.findByRole("heading",{name:"Perfil de rendimiento"});expect(heading.closest(".deny-profile")).toHaveClass("deny-reference")});
  ...
  it("uses accessible decorative SVG icons for primary navigation and keeps subsections icon-free", async () => {
    renderShell("/dashboard");
    await screen.findByRole("link",{name:"Salud"});
    ...
    for(const name of ["Carga de entrenamiento","Estado de forma","Cuenta","Perfil deportivo","Perfil de rendimiento"]){
      const link=screen.getByRole("link",{name});
  ...
  it("shows and activates Cuenta before Perfil de rendimiento",async()=>{renderShell("/settings/account");const accountLink=screen.getByRole("link",{name:"Cuenta"});expect(accountLink).toHaveAttribute("href","/settings/account");expect(accountLink).toHaveClass("nav-link--active");expect(await screen.findByRole("heading",{name:"Cuenta"})).toBeVisible();const labels=screen.getAllByRole("link").map(link=>link.textContent);expect(labels.indexOf("Cuenta")).toBeLessThan(labels.indexOf("Perfil deportivo"));expect(labels.indexOf("Perfil deportivo")).toBeLessThan(labels.indexOf("Perfil de rendimiento"))});

  it("refreshes the shell name and avatar after a successful account update",async()=>{const {client}=renderShell("/settings/account");vi.mocked(client.authMe).mockResolvedValue({id:"user-1",display_name:"Ana Nueva",email:"ana@example.test",authentication_mode:"session",account_plan:"owner"});const input=await screen.findByLabelText("Nombre visible");await userEvent.clear(input);await userEvent.type(input,"Ana Nueva");await userEvent.click(screen.getByRole("button",{name:"Guardar nombre"}));await waitFor(()=>expect(document.querySelector(".topbar__greeting strong")).toHaveTextContent("Ana Nueva"));expect(screen.getByLabelText("Usuario: Ana Nueva")).toHaveTextContent("A")});
...
await user.click(closeButton);
  ...
  it("lets a user without athletes create the first one",async()=>{renderShell("/dashboard",{...session,athletes:[],selected_athlete_id:null,selection_required:false});const trigger=await screen.findByRole("button",{name:"Crear mi primer atleta"});await userEvent.click(trigger);expect(screen.getByRole("dialog",{name:"Nuevo atleta"})).toBeInTheDocument()});  it("shows only athlete names for two owner memberships and keeps the account identity while switching both ways",async()=>{const owners={...session,athletes:[{...session.athletes[0],athlete_id:"athlete-a",label:"Carlos Murat",role:"owner"},{...session.athletes[0],athlete_id:"athlete-b",label:"Jenny Ruiz",role:"owner",is_default:false}],selected_athlete_id:"athlete-a"};renderShell("/dashboard",owners);const selector=await screen.findByRole("combobox",{name:"Seleccionar atleta"});expect(Array.from((selector as HTMLSelectElement).options).map(option=>option.textContent)).toEqual(["Selecciona…","Carlos Murat","Jenny Ruiz"]);expect(await screen.findByText("Propietario")).toBeVisible();expect(document.querySelector(".topbar__greeting strong")).toHaveTextContent("Ana");await userEvent.selectOptions(selector,"athlete-b");expect(selector).toHaveValue("athlete-b");expect(document.querySelector(".topbar__greeting strong")).toHaveTextContent("Ana");expect(screen.getByText("Propietario")).toBeVisible();await userEvent.selectOptions(selector,"athlete-a");expect(selector).toHaveValue("athlete-a");expect(screen.getByText("Propietario")).toBeVisible();expect(screen.queryByText("Carlos Murat · Propietario")).not.toBeInTheDocument();expect(screen.queryByText("Jenny Ruiz · Propietario")).not.toBeInTheDocument()});  it("renders a single owner athlete by profile name and keeps the membership label out of the selector",async()=>{renderShell("/dashboard",{...session,athletes:[{...session.athletes[0],label:"Jenny Ruiz",role:"owner"}]});expect((await screen.findAllByText("Jenny Ruiz")).length).toBeGreaterThanOrEqual(2);expect(screen.getByText("Propietario")).toBeVisible();expect(screen.queryByText("Jenny Ruiz · Propietario")).not.toBeInTheDocument()});});
describe("role capability navigation",()=>{
  it("hides health and denies a direct health route for coach capabilities",async()=>{
    const coach={...session,athletes:[{...session.athletes[0],role:"coach",capabilities:["read_athlete_data","read_strava_integration","disconnect_strava","run_strava_import"]}]};
    renderShell("/health",coach);
    expect(await screen.findByRole("heading",{name:"Acceso no permitido"})).toBeVisible();
    expect(screen.queryByRole("link",{name:"Salud"})).not.toBeInTheDocument();
  });
});
describe("dashboard membership identity",()=>{
  it("updates owner to coach immediately while selector options remain role-free",async()=>{
    const memberships={...session,athletes:[
      {...session.athletes[0],athlete_id:"athlete-owner",label:"Perfil Alfa",role:"owner",is_default:true},
      {...session.athletes[0],athlete_id:"athlete-coach",label:"Perfil Beta",role:"coach",is_default:false,capabilities:["read_athlete_data","disconnect_strava","run_strava_import"]},
    ],selected_athlete_id:"athlete-owner"};
    renderShell("/dashboard",memberships);
    expect(await screen.findByText("Propietario")).toBeVisible();
    const selector=screen.getByRole("combobox",{name:"Seleccionar atleta"});
    expect(Array.from((selector as HTMLSelectElement).options).map(option=>option.textContent)).toEqual(["Selecciona…","Perfil Alfa","Perfil Beta"]);
    expect(screen.queryByText("Perfil Alfa · Propietario")).not.toBeInTheDocument();
    await userEvent.selectOptions(selector,"athlete-coach");
    expect(await screen.findByText("Entrenador")).toBeVisible();
    expect(screen.queryByText("Propietario")).not.toBeInTheDocument();
    expect(Array.from((selector as HTMLSelectElement).options).map(option=>option.textContent)).toEqual(["Selecciona…","Perfil Alfa","Perfil Beta"]);
  });
});
describe("empty coach account",()=>{
  it("shows a stable Coach home without athlete-scoped navigation",async()=>{
    renderShell("/dashboard",{...session,user:{...session.user,display_name:"Juan Entrenador",account_plan:"coach"},athletes:[],selected_athlete_id:null,selection_required:false});
    expect(await screen.findByRole("heading",{name:"Bienvenido, Juan Entrenador"})).toBeVisible();
    expect(screen.getByText("Entrenador")).toBeVisible();
    expect(screen.getByText("Todavía no tienes atletas asignados.")).toBeVisible();
    expect(screen.queryByRole("combobox",{name:"Seleccionar atleta"})).not.toBeInTheDocument();
    expect(screen.queryByRole("link",{name:"Actividades"})).not.toBeInTheDocument();
    expect(screen.getByRole("link",{name:"Cuenta"})).toBeVisible();
