...
export function AppShell({client=apiClient,showLogout=false,onLogout}:{client?:ApiClient;showLogout?:boolean;onLogout?():Promise<void>}){
 ...
 const {athletes,activeAthleteId,loading,selectionRequired,error,announcement,hasAthleteCapability,refreshContext}=useAthleteContext();
 ...
 const content=path==="/settings/account"?routeContent(path,client,hasAthleteCapability):loading?<section role="status">Cargando contexto…</section>:error?<section role="alert"><h1>Error al cargar el contexto</h1><p>{error}</p></section>:!athletes.length?<section className="athlete-empty"><h1>Sin atleta asociado</h1><p>No existe un perfil de atleta asociado a este usuario.</p><button type="button" className="button button--primary" onClick={event=>openCreate(event.currentTarget)}>Crear mi primer atleta</button></section>:selectionRequired?<section className="athlete-selection-screen"><h1>Selección de atleta necesaria</h1><p>Selecciona el perfil con el que quieres trabajar.</p><AthleteSelector required onCreate={openCreate}/></section>:activeAthleteId?<div key={activeAthleteId}>{routeContent(path,client,hasAthleteCapability)}</div>:null;
 return <div className="application-shell">
  <a className="skip-link" href="#main-content">Saltar al contenido principal</a>
  <aside className={`sidebar ${drawer?"sidebar--open":""}`} aria-label="Navegación principal"><div className="brand"><span className="brand__mark" aria-hidden="true">T</span><span>TriCoach AI</span></div><nav>{nav.map(item=><span key={item.path}><AppLink to={item.path} className={`nav-link ${path===item.path||(item.path==="/activities"&&path.startsWith("/activities/"))?"nav-link--active":""}`}><NavIcon name={item.icon}/><span className="nav-item-label">{item.label}</span></AppLink>{item.path==="/activities"&&<AppLink to="/activities/strength" className={`nav-link nav-link--strength ${path==="/activities/strength"?"nav-link--active":""}`}><NavIcon name="strength"/><span className="nav-item-label">Fuerza</span></AppLink>}{item.path==="/statistics"&&<><AppLink to="/statistics/training-load" className={`nav-link nav-link--nested nav-subitem ${path==="/statistics/training-load"?"nav-link--active":""}`}>Carga de entrenamiento</AppLink><AppLink to="/statistics/training-status" className={`nav-link nav-link--nested nav-subitem ${path==="/statistics/training-status"?"nav-link--active":""}`}>Estado de forma</AppLink></>}{item.path==="/settings"&&<><AppLink to="/settings/account" className={`nav-link nav-link--nested nav-subitem ${path==="/settings/account"?"nav-link--active":""}`}>Cuenta</AppLink><AppLink to="/settings/performance-profile" className={`nav-link nav-link--nested nav-subitem ${path==="/settings/performance-profile"?"nav-link--active":""}`}>Perfil de rendimiento</AppLink></>}</span>)}</nav><p className="sidebar__version">Versión 0.7G.5</p></aside>
  {drawer&&<button className="drawer-backdrop" aria-label="Cerrar navegación" onClick={()=>setDrawer(false)}/>}
  <div className="application-main"><header className="topbar"><button className="icon-button mobile-menu" type="button" aria-label={drawer?"Cerrar menú":"Abrir menú"} aria-expanded={drawer} onClick={()=>setDrawer(value=>!value)}>☰</button>{activeAthleteId&&<ActivitySyncButton client={client}/>}<AthleteSelector onCreate={openCreate}/><div className="topbar__greeting"><strong>{greeting()}, {displayName}</strong><span>{date}</span></div><div className="topbar__actions"><button className="icon-button" type="button" aria-label={`Activar tema ${theme==="light"?"oscuro":"claro"}`} onClick={toggle}>{theme==="light"?"☾":"☀"}</button><div className="user-menu" ref={menuRef}><button className="user-button" type="button" aria-haspopup="menu" aria-expanded={userOpen} onClick={()=>setUserOpen(value=>!value)}><span className="avatar" aria-label={`Usuario: ${displayName}`}>{initial}</span><span>{displayName}</span><span aria-hidden="true">⌄</span></button>{userOpen&&<div className="user-menu__panel" role="menu"><button role="menuitem" disabled>Perfil <small>Próximamente</small></button><AppLink to="/settings/account" className="user-menu__link">Cuenta</AppLink><AppLink to="/settings/performance-profile" className="user-menu__link">Perfil de rendimiento</AppLink>{showLogout?<button role="menuitem" onClick={()=>void onLogout?.()}>Cerrar sesión</button>:<button role="menuitem" disabled>Cerrar sesión <small>Desarrollo</small></button>}</div>}</div></div></header><div className="sr-only" aria-live="polite">{announcement}</div><main id="main-content" className="page-content" tabIndex={-1}>{path==="/"?null:content}</main></div>{creating&&<NewAthleteDialog client={client} refreshContext={refreshContext} refreshUser={refreshUser} onClose={closeCreate} onActivated={()=>navigate("/dashboard")}/>}
 </div>
}
