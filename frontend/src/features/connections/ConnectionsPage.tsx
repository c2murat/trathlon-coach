...
export function StravaConnectionPanel({client=apiClient,athleteId,athleteName,canManage=false,canSync=false,onExternalNavigate=(url)=>window.location.assign(url)}:Required<Pick<Props,"athleteId"|"athleteName">>&Props){
 ...
 <div className="connection-actions">{canManage&&(!status.connected||status.requires_reconnect)&&<button className="button button--primary" type="button" disabled={busy} onClick={()=>void connect()} aria-label={`${status.requires_reconnect?"Reconectar":"Conectar"} Strava para ${athleteName}`}>{busy?"Abriendo Strava…":status.requires_reconnect?"Reconectar Strava":"Conectar Strava"}</button>}{status.connected&&canSync&&<ActivitySyncButton client={client}/>} {canManage&&status.connected&&!confirming&&<button className="button button--ghost" type="button" disabled={busy} onClick={()=>setConfirming(true)}>Desconectar Strava</button>}</div>
