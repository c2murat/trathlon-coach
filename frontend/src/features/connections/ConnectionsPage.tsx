...
type Props={client?:ApiClient;athleteId?:string;athleteName?:string;canManage?:boolean;canSync?:boolean;onExternalNavigate?:(url:string)=>void};
...
export function ConnectionsPage(props:Props){
 ...
 const canManage=props.canManage??context.hasAthleteCapability(ATHLETE_CAPABILITIES.MANAGE_STRAVA),canSync=props.canSync??context.hasAthleteCapability(ATHLETE_CAPABILITIES.IMPORT_STRAVA);
 return <StravaConnectionPanel {...props} athleteId={athleteId} athleteName={athleteName} canManage={canManage} canSync={canSync}/>;
}

export function StravaConnectionPanel({client=apiClient,athleteId,athleteName,canManage=false,canSync=false,onExternalNavigate=(url)=>window.location.assign(url)}:Required<Pick<Props,"athleteId"|"athleteName">>&Props){
 ...
 <div className="connection-actions">{canManage&&(!status.connected||status.requires_reconnect)&&<button className="button button--primary" type="button" disabled={busy} onClick={()=>void connect()} aria-label={`${status.requires_reconnect?"Reconectar":"Conectar"} Strava para ${athleteName}`}>{busy?"Abriendo Strava…":status.requires_reconnect?"Reconectar Strava":"Conectar Strava"}</button>}{status.connected&&canSync&&<ActivitySyncButton key={athleteId} athleteId={athleteId} client={client}/>} {canManage&&status.connected&&!confirming&&<button className="button button--ghost" type="button" disabled={busy} onClick={()=>setConfirming(true)}>Desconectar Strava</button>}</div>
 ...
 {!canManage&&<p className="connection-readonly">Puedes consultar esta conexión, pero no gestionarla.</p>}</>}
 </section></article>;
}
