...
export function TrainingStatusPage({ client, canRecalculate=true, athleteId }: { client: ApiClient; canRecalculate?:boolean; athleteId?:string }) {
  ...
  const [interpretation,setInterpretation]=useState<TrainingStatusInterpretation|null>(null);
  ...
  const scope=`${athleteId??"legacy"}:${weeks}`;
  const waiting=loading||loadedScope!==scope;

  useEffect(()=>{
    const id=++requestId.current;
    setLoading(true);setQueryError(false);setNoSources(false);
    setRows([]);setLatest(null);setInterpretation(null);setRecalculationError("");setAnnouncement("");setRecalculating(false);
    const load=async()=>{
      if(client.trainingStatusOverview){
        const result=await client.trainingStatusOverview(query);
        if(athleteId&&result.interpretation.athlete_id!==athleteId)throw new Error("athlete mismatch");
        return result;
      }
      if(!client.listTrainingStatus||!client.getLatestTrainingStatus)throw new Error("status unavailable");
      const [series,current]=await Promise.all([client.listTrainingStatus(query),client.getLatestTrainingStatus(query)]);
      return {series,latest:current,interpretation:null};
    };
    void load()
      .then(result=>{if(id===requestId.current){setRows(result.series);setLatest(result.latest);setInterpretation(result.interpretation)}})
      .catch(()=>{if(id===requestId.current)setQueryError(true)})
      .finally(()=>{if(id===requestId.current){setLoadedScope(scope);setLoading(false)}});
    return ()=>{requestId.current++};
  },[client,weeks,athleteId]);
  ...
  async function recalculate(){
    ...
    const id=requestId.current;
    try{
      let series=await client.recalculateTrainingStatus(query);
      let current:DailyTrainingStatus|null;
      if(client.trainingStatusOverview){
        const result=await client.trainingStatusOverview(query);
        if(id!==requestId.current)return;
        if(athleteId&&result.interpretation.athlete_id!==athleteId)throw new Error("athlete mismatch");
        series=result.series;current=result.latest;setInterpretation(result.interpretation);
      }else current=client.getLatestTrainingStatus?await client.getLatestTrainingStatus(query):(series.at(-1)??null);
      if(id!==requestId.current)return;
    ...
    }catch{if(id===requestId.current)setRecalculationError("No se ha podido recalcular el estado de entrenamiento.");}
    finally{if(id===requestId.current)setRecalculating(false)}
  }
  ...
  return <article className="training-status-page">
    ...
    {waiting&&<section className="training-load-state" role="status"><strong>Cargando estado de entrenamiento…</strong></section>}
    {!waiting&&queryError&&<section className="training-load-state training-load-state--error" role="alert"><strong>No se ha podido consultar el estado de entrenamiento.</strong></section>}
    {!waiting&&!queryError&&globallyEmpty&&!noSources&&<section className="training-load-state status-empty"><h2>Todavía no hay un estado de entrenamiento calculado</h2><p>Calcula el estado para obtener la evolución del fitness, la fatiga y la forma a partir de tu carga diaria.</p>{canRecalculate&&<button className="button button--primary" disabled={recalculating} onClick={()=>void recalculate()}>Calcular estado de entrenamiento</button>}</section>}
    {!waiting&&!queryError&&noSources&&<section className="training-load-state status-empty"><h2>No hay carga de entrenamiento disponible</h2><p>Registra o importa entrenamientos antes de calcular el estado.</p></section>}
    {!waiting&&!queryError&&!latest&&interpretation&&<TrainingStatusSummary interpretation={interpretation}/>}
    {recalculationError&&<p className="status-recalculation-error" role="alert" aria-live="assertive">{recalculationError}</p>}
    {!waiting&&!queryError&&latest&&<>
      ...
      {interpretation&&<TrainingStatusSummary interpretation={interpretation}/>}
    ...
    {!waiting&&!queryError&&!globallyEmpty&&latest&&rows.length===0&&<p className="status-interval-empty">No hay estados en el intervalo seleccionado; se muestra el estado más reciente.</p>}
  </article>;
}
