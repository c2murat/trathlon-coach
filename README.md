...
## Version 0.5A ï¿½ Athlete dashboard
...
- `GET /dashboard/trends?weeks=8` (4ï¿½52 ISO weeks, oldest to newest)
- `GET /dashboard/consistency?weeks=12` (4ï¿½52 weeks)
...
## Version 0.5A.1 ï¿½ UX and dashboard polish
...
Routes: `/` redirects to `/dashboard`; `/dashboard` is operational; `/activities`, `/calendar`, `/statistics`, `/health`, and `/settings` display honest `Prï¿½ximamente` pages. Theme selection defaults to the system preference and persists under `tricoach-theme` in `localStorage`.
...
## Version 0.5B ï¿½ Activity browsing foundations
...
- `GET http://127.0.0.1:8000/activities` ï¿½ offset pagination (20 by default, 100 maximum) and optional `sport`, `date_from`, `date_to`, `min_distance_metres`, `max_distance_metres`, `trainer`, `manual`, `visibility`, and `search` filters.
- `GET http://127.0.0.1:8000/activities/filter-options` ï¿½ athlete-owned available sports, visibility values, and activity date bounds.
- `GET http://127.0.0.1:8000/activities/{activity_id}` ï¿½ athlete-owned allow-listed imported summary or 404.
...
## Version 0.5B.1 ï¿½ Activity summary display polish
...
Visibility, trainer, manual-entry, and commute metadata are localized into readable Spanish phrases. The detail view is organized into **Resumen**, **Rendimiento**, and **Informaciï¿½n de la actividad**.
...
## Version 0.5B.2 ï¿½ Activity detail final polish
...
## Version 0.6A ï¿½ Strava activity detail enrichment
...
## Version 0.6B ï¿½ Laps, streams, and factual evidence
...
Next sprint: Version 0.6C ï¿½ deterministic factual metrics and coverage rules. It is not part of Version 0.6B and has not begun.


## Version 0.6C ï¿½ mï¿½tricas factuales

La primera implementaciï¿½n de 0.6C calcula mï¿½tricas deterministas sobre actividades ya importadas. Cada resultado conserva estado (`available`, `partial`, `unavailable`, `not_applicable`), unidad, fuente, muestras, cobertura, notas y motivo seguro de indisponibilidad. El algoritmo estable es `0.6c.1`.

La persistencia usa `activity_metrics`, relacionada con `completed_activities`, con unicidad por actividad, mï¿½trica y versiï¿½n. Los endpoints son `GET /activities/{activity_id}/metrics` y `POST /activities/{activity_id}/metrics/recalculate`; el recï¿½lculo es aislado e idempotente.

Incluye tiempo, distancia, desnivel, velocidades, frecuencia cardiaca, potencia, cadencia, vueltas y muestras de streams. Las fuentes son resumen almacenado y streams disponibles; no se interpolan streams ausentes o desalineados. Los valores no numï¿½ricos, NaN e infinitos se descartan y nunca se convierten silenciosamente en cero.

Quedan fuera TSS, TRIMP, NP, IF, VI, GAP, SWOLF, zonas, umbrales, carga, recuperaciï¿½n, fatiga, readiness, coaching, recomendaciones, planificaciï¿½n e IA. No hay recï¿½lculo masivo automï¿½tico ni worker independiente.

### Cierre 0.7A.2
Perfil de rendimiento histï¿½rico, API mï¿½nima, zonas dinï¿½micas y CORS local validados. 0.7A.3 se centrarï¿½ en gestiï¿½n avanzada del perfil y referencias por deporte; no incluye motor de carga ni entrenador IA.

## 0.7A.3B ï¿½ Referencias avanzadas por deporte
...
## VersiÃ³n 0.7F.5 â€” Backfill y sincronizaciÃ³n del estado de entrenamiento

El estado diario (Fitness, Fatiga y Forma) se sincroniza automÃ¡ticamente, dentro de la misma transacciÃ³n, despuÃ©s de recalcular agregados diarios o completos y despuÃ©s de crear, editar, eliminar o recalcular una sesiÃ³n manual de fuerza. El recÃ¡lculo semanal aislado no modifica el estado diario.

Backfill histÃ³rico seguro desde `backend`:
...
Por defecto usa `Europe/Madrid`, carga `0.7b.1`, fuerza manual `0.7e.1` y la versiÃ³n vigente del estado. Sin fechas, comienza en la primera carga diaria compatible y alcanza el dÃ­a local actual. Para limitarlo, deben proporcionarse juntas `--start-date YYYY-MM-DD` y `--end-date YYYY-MM-DD`. `--dry-run` ejecuta el cÃ¡lculo completo y muestra el resumen, pero revierte toda la transacciÃ³n; la ejecuciÃ³n real realiza un Ãºnico commit al finalizar. El proceso es idempotente y no modifica las cargas fuente.
