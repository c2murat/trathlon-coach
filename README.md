...
## Version 0.4 — First usable web interface
...
Routes: `/` redirects to `/dashboard`; `/dashboard` is operational; `/activities`, `/calendar`, `/statistics`, `/health`, and `/settings` display honest `Pr�ximamente` pages. Theme selection defaults to the system preference and persists under `tricoach-theme` in `localStorage`.
...
Visibility, trainer, manual-entry, and commute metadata are localized into readable Spanish phrases. The detail view is organized into **Resumen**, **Rendimiento**, and **Informaci�n de la actividad**.
...
## Version 0.6C � m�tricas factuales

La primera implementaci�n de 0.6C calcula m�tricas deterministas sobre actividades ya importadas. Cada resultado conserva estado (`available`, `partial`, `unavailable`, `not_applicable`), unidad, fuente, muestras, cobertura, notas y motivo seguro de indisponibilidad. El algoritmo estable es `0.6c.1`.

La persistencia usa `activity_metrics`, relacionada con `completed_activities`, con unicidad por actividad, m�trica y versi�n. Los endpoints son `GET /activities/{activity_id}/metrics` y `POST /activities/{activity_id}/metrics/recalculate`; el rec�lculo es aislado e idempotente.

Incluye tiempo, distancia, desnivel, velocidades, frecuencia cardiaca, potencia, cadencia, vueltas y muestras de streams. Las fuentes son resumen almacenado y streams disponibles; no se interpolan streams ausentes o desalineados. Los valores no num�ricos, NaN e infinitos se descartan y nunca se convierten silenciosamente en cero.

Quedan fuera TSS, TRIMP, NP, IF, VI, GAP, SWOLF, zonas, umbrales, carga, recuperaci�n, fatiga, readiness, coaching, recomendaciones, planificaci�n e IA. No hay rec�lculo masivo autom�tico ni worker independiente.

### Cierre 0.7A.2
Perfil de rendimiento hist�rico, API m�nima, zonas din�micas y CORS local validados. 0.7A.3 se centrar� en gesti�n avanzada del perfil y referencias por deporte; no incluye motor de carga ni entrenador IA.

## 0.7A.3B � Referencias avanzadas por deporte

Las referencias granulares se almacenan hist�ricamente en thlete_performance_references; los snapshots 0.7A.2 permanecen como hist�rico agregado durante la transici�n. Las nuevas referencias granulares son la fuente de verdad para nuevas altas. Las zonas contin�an calcul�ndose bajo demanda y no se persisten.

Endpoints: GET/POST /athlete/performance-references, GET /history y GET /{reference_id}. Soportan filtros por deporte, m�trica y fecha efectiva.

## Versi�n 0.7B.1 � Dominio de carga

Se a�adi� un dominio puro y determinista para estimar puntos de carga por actividad. Incluye m�todos de potencia ciclista, frecuencia cardiaca, ritmo de carrera, CSS de nataci�n y fallback conservador por duraci�n. No incluye persistencia, API, frontend, TSS oficial, TRIMP, CTL, ATL, TSB ni entrenador.

## Versi�n 0.7B.2 � Persistencia de carga por actividad

La carga determinista 0.7B.1 puede persistirse por actividad con algoritmo versionado e idempotencia por actividad y versi�n. Se selecciona el perfil hist�rico m�s reciente con `effective_from` anterior o igual al inicio de la actividad. No hay endpoints ni frontend; CTL, ATL, TSB, TSS/TRIMP oficiales y agregados permanecen fuera de alcance.

## Versi�n 0.7B.3 � API de carga por actividad

Se exponen `GET /activities/{activity_id}/training-load` para consultar �nicamente cargas persistidas y `POST /activities/{activity_id}/training-load/recalculate` para calcular o recalcular expl�citamente. El GET nunca calcula; una carga puede ser nula con cobertura y motivo expl�citos. Se mantiene el aislamiento por atleta, el perfil hist�rico vigente y la idempotencia por versi�n de algoritmo. No hay frontend, CTL, ATL, TSB, agregados, entrenador ni IA.

## Versi�n 0.7C.1 � Agregaci�n temporal pura

Se a�adi� el dominio inmutable para agregados diarios y semanales de cargas ya calculadas. Usa fechas locales IANA y semanas ISO, valida versiones estrictamente, ignora cargas nulas al sumar pero las contabiliza, y no persiste ni expone API. CTL, ATL, TSB y series continuas permanecen fuera de alcance.

## Versi�n 0.7C.2 � Persistencia de agregados de carga

Se a�adieron agregados diarios y semanales hist�ricos a partir exclusivamente de `ActivityTrainingLoad`. Los agregados se guardan con zona horaria IANA, versi�n de carga fuente y versi�n de agregaci�n, cobertura, calidad, advertencias e identificadores de actividad. Las claves �nicas permiten coexistencia segura por configuraci�n.

El rec�lculo diario usa un intervalo local inclusivo convertido a UTC mediante l�mite superior exclusivo. El rec�lculo semanal expande el rango a lunes-domingo ISO completos. No se persisten periodos vac�os; s� se conservan periodos con actividades y cargas nulas. Las actividades sin carga persistida quedan excluidas y las cargas cero se conservan. Las operaciones son idempotentes, actualizan filas existentes y eliminan sobrantes �nicamente dentro de la configuraci�n y rango afectados.
