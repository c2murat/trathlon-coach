...
Solo OWNER puede gestionar Coaches y memberships, y únicamente en el `AthleteProfile` seleccionado donde tenga `manage_athlete_memberships`. ATHLETE no busca, añade ni elimina entrenadores. COACH no busca atletas globalmente: el selector lista exclusivamente memberships activas devueltas por `/session/context`.
